...
1. **One-letter elements generate an invalid internal setup ID** — [strc.py:148](../backend/src/atomscope/backends/cppaw/strc.py#L148)
   ...
   Fix: construct `ID` as `f"{species_name(sym)}{opts.setup_type}"`, without inserting another underscore. For the empty fallback at [strc.py:158](../backend/src/atomscope/backends/cppaw/strc.py#L158), use `H_.75_6.0`.

2. **“Restart from parent” copies a restart file but still generates `START=T` by default** — [service.py:156](../backend/src/atomscope/calculations/service.py#L156), [cntl.py:49](../backend/src/atomscope/backends/cppaw/cntl.py#L49)
...
3. **Cancellation defeats CP-PAW’s exit-file soft-stop protocol** — [manager.py:97](../backend/src/atomscope/jobs/manager.py#L97), [runner.py:52](../backend/src/atomscope/backends/cppaw/runner.py#L52)
...
4. **Incomplete CP-PAW runs can be recorded as completed** — [runner.py:73](../backend/src/atomscope/backends/cppaw/runner.py#L73), [manager.py:159](../backend/src/atomscope/jobs/manager.py#L159), [service.py:46](../backend/src/atomscope/calculations/service.py#L46)
   ...
   Evidence: the health-check correctly requires both code zero and `PROGRAM FINISHED` at [settings.py:146](../backend/src/atomscope/backends/cppaw/settings.py#L146); the analysis requires the same predicate at `cppaw-analysis.md:1706-1708`.
...
5. **Antiferromagnetic and state-controlled occupations are not representable** — [strc.py:131](../backend/src/atomscope/backends/cppaw/strc.py#L131), [strc.py:246](../backend/src/atomscope/backends/cppaw/strc.py#L246)
...
6. **The runtime fallback is implemented but not invoked for normal launches** — [settings.py:176](../backend/src/atomscope/backends/cppaw/settings.py#L176), [plugin.py:148](../backend/src/atomscope/backends/cppaw/plugin.py#L148)

   `ensure_runtime()` can detect the known libgfortran failure and select a compatible runtime, but `run_spec()` never calls it. A first normal calculation can therefore fail on this workstation despite the implemented remedy. The existing test expects the missing behavior at [test_plugin.py:102](../backend/tests/backends/cppaw/test_plugin.py#L102).
...
7. **Periodic cube view boxes corrupt non-orthogonal cell geometry** — [plugin.py:195](../backend/src/atomscope/backends/cppaw/plugin.py#L195), [cntl.py:183](../backend/src/atomscope/backends/cppaw/cntl.py#L183)
...
8. **Deck parsing changes duplicate-key semantics** — [deck.py:119](../backend/src/atomscope/backends/cppaw/deck.py#L119)
...
9. **Trajectory “last run only” detection fails for normal restart continuations** — [tra.py:58](../backend/src/atomscope/backends/cppaw/tra.py#L58)
...
10. **The default force task runs 305 damped ionic steps, not the advertised five** — [cntl.py:59](../backend/src/atomscope/backends/cppaw/cntl.py#L59)

   For forces, `NSTEP` is `nstep + force_steps`; defaults therefore become `300 + 5`. It also enables RDYN from scratch, so nuclei move before the electronic state is necessarily converged. The test explicitly expects this at [test_strc_cntl.py:85](../backend/tests/backends/cppaw/test_strc_cntl.py#L85).
...
11. **Root and cube arguments are treated as filesystem paths without local validation** — [runner.py:41](../backend/src/atomscope/backends/cppaw/runner.py#L41)
...
12. **A molecular explicit k-point grid is silently ignored** — [plugin.py:86](../backend/src/atomscope/backends/cppaw/plugin.py#L86), [strc.py:120](../backend/src/atomscope/backends/cppaw/strc.py#L120)
