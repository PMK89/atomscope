...
| `backend/tests/fixtures/qc_outputs/{benzene.g03,methane.g03,methane.nwo}`, `backend/tests/fixtures/wavefunction/{d-only,f-only}.gamess.gz`, `backend/tests/fixtures/wavefunction/caffeine_orca.{molden,out}.gz` (trimmed), `backend/tests/fixtures/wavefunction/methane.mpo.gz` | Avogadro 1 `testfiles/` | GPL-2.0-or-later, and the BSD-3-Clause notice that directory ships | parser and wavefunction-reader test fixtures |
