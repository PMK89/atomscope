...
Branch: main; this file is updated in the commit that checkpoints the work, so `git log -1 -- docs/STATE.md` is the last checkpoint. Phases 0-1 done; Phase 2 (editor tools), 3 (volumetric, trajectories, vectors), 4-5 (CP-PAW setup/execution/forces), 6 (CP-PAW analysis: DOS, bands, orbitals), crystallography, molecular mechanics and wavefunction surfaces are merged and working. Parity matrix: 193 IMPLEMENTED, 36 PARTIAL, 82 NOT STARTED, 1 BLOCKED of 312 rows.
...
3. What is left at HIGH or CRITICAL, from the matrix itself (an earlier version of this list said
   ...
   Today that prints **18 rows, all PARTIAL** -- no CRITICAL or HIGH row is NOT STARTED, which is
   ...
   Run the same awk with `MEDIUM` for what is next: today it lists 47 rows, of which the ones with
   a real workflow behind them are PDB/name fetch (AV-FILE-013/014, AV-BIO-010), recent files
   (AV-FILE-007) and drag-and-drop open (AV-FILE-010).
