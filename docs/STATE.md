...
Branch: main; this file is updated in the commit that checkpoints the work, so `git log -1 -- docs/STATE.md` is the last checkpoint. Phases 0-1 done; Phase 2 (editor tools), 3 (volumetric, trajectories, vectors), 4-5 (CP-PAW setup/execution/forces), 6 (CP-PAW analysis: DOS, bands, orbitals), crystallography, molecular mechanics and wavefunction surfaces are merged and working. Parity matrix: 175 IMPLEMENTED, 34 PARTIAL, 102 NOT STARTED, 1 BLOCKED of 312 rows.

Tests: `pytest -q -m "not cppaw"` -> 376 passed, 1 skipped; `pytest -q -m cppaw` -> 7 passed (~90 s, needs the local CP-PAW install); `pnpm vitest run` -> 380 passed; `pnpm exec playwright test` -> 24 passed (against private servers, see below; `make test-e2e` points at the user's 5173, which is stale). `ruff check`, `mypy` and `pnpm typecheck` are clean. No known failing tests. **Type-check the frontend with `pnpm typecheck` (`tsc -b --noEmit`), never with `pnpm exec tsc --noEmit`:** the root `tsconfig.json` is a solution file with `files: []`, so a bare `tsc --noEmit` checks nothing and exits 0.
...
- This checkpoint (Avogadro parity, editor and rendering): a surface coloured by a second grid with
  ...
  dialog over every format the backend can write, which refuses to overwrite until asked twice. Fixes found on the way: Optimize geometry sent valueless force-field
...
3. What is left at HIGH or CRITICAL, from the matrix itself (an earlier version of this list said
   ...
   Today that is two HIGH `NOT STARTED`: a painter abstraction for POV-Ray/VRML export
   (AV-VIS-042) and the MOPAC input generator (AV-QM-007). Nothing CRITICAL is left open -- the
