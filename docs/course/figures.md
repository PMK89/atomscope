...
| 3.1 | 3.3 | total DOS with the oxygen-p and other projections stacked underneath it | `paw_dos.x` → xmgrace | Analysis ▸ DOS, projected per element or atom | PARTIAL |
| 4.1 | 4.1 | proton tunnelling in malonaldehyde — the reaction, as a scheme | drawn by hand | — | n/a |
| 4.2 | 4.3 | total DOS plus a stack of the hydrogen, carbon and oxygen projections | `paw_dos.x` → xmgrace | Analysis ▸ DOS | PARTIAL |
...
| 4.6 | 4.4 | the same DOS stack for the relaxed (asymmetric) geometry | `paw_dos.x` → xmgrace | Analysis ▸ DOS | PARTIAL |
...
| 6.1 | 6.2 | DOS of silicon: total as an outline, the projections as filled regions | `paw_dos.x` → xmgrace | Analysis ▸ DOS | PARTIAL |
...
| 6.8 | 6.4 | DOS of aluminium against the free-electron-gas √E curve | `paw_dos.x` + an analytic curve | Analysis ▸ DOS | PARTIAL |
...
| 7.1 | 7.1 | spin-resolved DOS of α-iron, majority against minority | `paw_dos.x` → xmgrace | Analysis ▸ DOS, spin channels mirrored about zero | PARTIAL |
...
| 7.3 | 7.2 | DOS of NiO, total and angular-momentum projected, per spin | `paw_dos.x` → xmgrace | Analysis ▸ DOS with `!ORB` weights | PARTIAL |
| 8.1 | 8.1 | two panels: total energy above, plane-wave counts below | two xmgrace graphs | Sweeps panel draws the energy; the counts are collected but not drawn | PARTIAL |
...
**Totals: 10 SHOWS · 15 PARTIAL · 5 MISSING · 5 n/a.**
...
| Gap | Figures | Size |
|-----|---------|------|
| stacked, filled DOS with the occupied side shaded (see below) | 3.1, 4.2, 4.6, 6.1, 6.8, 7.1, 7.3 | one chart mode — the data is already in `DosSeries` |
| band curves coloured by occupation | 6.4, 6.9 | needs the Fermi level per band point; `paw_bands.x` gives eigenvalues only |
| a second panel under a sweep chart | 8.1 | `SweepPoint.properties` already carries the plane-wave counts |
