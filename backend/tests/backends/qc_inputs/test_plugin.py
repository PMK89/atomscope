...
def test_a_dalton_run_is_the_pair_of_files_dalton_reads() -> None:
    ...
    water = from_atoms(molecule("H2O"), name="water")
    files = plugin.generate_inputs(water, {"program": "dalton"}, "case").files
    assert [(f.name, f.role) for f in files] == [("case.dal", "input"), ("case.mol", "structure")]
    assert files[0].text == (
        "**DALTON INPUT\n.RUN WAVE FUNCTIONS\n**WAVE FUNCTIONS\n.HF\n**END OF DALTON INPUT\n"
    )
    assert files[1].text == (
        ...
        # runs of equal atomic number, each with its own Charge block
        "Charge=8.0 Atoms=1\n"
...
def test_the_dalton_basis_family_and_its_switches_pick_one_list() -> None:
    """`generateInputDeck:355-401` chooses between eleven lists by a family and three switches --
    and its aug-cc-pCV5Z entry fell through to aug-cc-pCVDZ."""
    water = from_atoms(molecule("H2O"), name="water")

    def basis(**values: object) -> str:
        files = plugin.generate_inputs(water, {"program": "dalton", **values}, "c").files
        return files[1].text.split("\n")[1]

    assert basis() == "STO-2G"
    assert basis(dalton_family="pople") == "3-21G"
    assert basis(dalton_family="pople", dalton_polarized=True) == "3-21G*"
    assert basis(dalton_family="pople", dalton_diffuse=True) == "3-21++G"
    assert basis(dalton_family="pople", dalton_polarized=True, dalton_diffuse=True) == "3-21++G*"
    assert basis(dalton_family="jensen") == "pc-0"
    assert basis(dalton_family="jensen", dalton_diffuse=True) == "apc-0"
    assert basis(dalton_family="dunning") == "cc-pVDZ"
    assert basis(dalton_family="dunning", dalton_core=True) == "cc-pCVDZ"
    assert basis(dalton_family="dunning", dalton_diffuse=True) == "aug-cc-pVDZ"
    # the augmentation prefixes the augmented set, and the core one is a list of its own
    assert (
        basis(
            dalton_family="dunning",
            dalton_diffuse=True,
            dalton_core=True,
            dalton_augmentation="triple",
            dalton_aug_cc_core="aug-cc-pCV5Z",
        )
        == "t-aug-cc-pCV5Z"
    )


def test_a_dalton_property_run_and_its_wave_function() -> None:
    """MP2 asks for its Hartree-Fock reference above itself, a grid other than the normal one
    opens `*DFT INPUT`, and a property run changes the run type at the top of the file."""
    water = from_atoms(molecule("H2O"), name="water")

    def deck(**values: object) -> str:
        return plugin.generate_inputs(water, {"program": "dalton", **values}, "c").files[0].text

    assert "**WAVE FUNCTIONS\n.HF\n.MP2\n" in deck(dalton_theory="mp2")
    plain_dft = deck(dalton_theory="dft", dalton_functional="CAMB3LYP")
    assert "**WAVE FUNCTIONS\n.DFT\n CAMB3LYP\n" in plain_dft
    assert "*DFT INPUT" not in plain_dft
    assert "*DFT INPUT\n.ULTRAFINE\n" in deck(dalton_theory="dft", dalton_grid="ultrafine")
    assert ".RUN PROPERTIES\n" in deck(dalton_property="polarizability")
    assert "**PROPERTIES\n.POLARI\n" in deck(dalton_property="polarizability")
    assert "**PROPERTIES\n.EXCITA\n 4\n" in deck(dalton_property="excitation", dalton_excitations=4)
    assert ".DIRECT\n.PARALLEL\n" in deck(dalton_direct=True, dalton_parallel=True)


def test_dalton_switches_symmetry_off_only_for_the_run_that_asks() -> None:
    """The geometry line asked whether the property was an excitation whatever the calculation
    type was (`:428-432`), so a stored choice leaked `Nosymm` into a plain wave-function run."""
    water = from_atoms(molecule("H2O"), name="water")

    def molecule_file(**values: object) -> str:
        return plugin.generate_inputs(water, {"program": "dalton", **values}, "c").files[1].text

    assert "Angstrom Nosymm\n" in molecule_file(dalton_property="excitation")
    assert "Angstrom\n" in molecule_file(dalton_property="polarizability")
    assert "Angstrom\n" in molecule_file()
...
def test_dalton_has_no_geometry_optimization_and_says_so() -> None:
    ...
    water = from_atoms(molecule("H2O"), name="water")
    for task in ("optimize", "frequencies", "transition_state"):
        issues = plugin.validate(water, {"program": "dalton", "task": task}).issues
        assert "task" in [i.key for i in issues], task
        with pytest.raises(ValueError, match="no geometry optimization"):
            plugin.generate_inputs(water, {"program": "dalton", "task": task}, "c")
    assert not plugin.validate(water, {"program": "dalton"}).issues
