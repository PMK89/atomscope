...
def test_orbital_labels_and_listing() -> None:
    assert [orbital_label(b, 4) for b in (2, 3, 4, 5, 6)] == ["HOMO-2", "HOMO-1", "HOMO", "LUMO", "LUMO+1"]
...
def test_orbital_restart_cntl() -> None:
    ...
    text = orbital_cntl_text("case", values, [OrbitalRequest(band=3), OrbitalRequest(band=4, kpoint=2, spin=2)])
